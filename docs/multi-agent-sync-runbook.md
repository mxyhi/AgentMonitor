...
Keep AgentMonitor's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between AgentMonitor settings and Codex runtime behavior
...
Run from `AgentMonitor` repo root:
...
## AgentMonitor Files To Update If Upstream Changes
...
- Keep AgentMonitor scoped to the new key only (no legacy alias read/write).
- Keep AgentMonitor writes aligned with upstream expectations.
...
- Reconcile AgentMonitor guardrails when upstream changes.
...
- AgentMonitor default `agents.max_depth` is `1`.
- AgentMonitor currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.

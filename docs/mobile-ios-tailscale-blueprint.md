# Agent Monitor iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted Agent Monitor backend over Tailscale.
...
1. Desktop Agent Monitor hosts the daemon and executes Codex workflows.
2. iOS Agent Monitor connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop Agent Monitor is installed and able to run local workspaces.

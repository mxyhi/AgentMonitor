...
describe("Markdown file-like href behavior", () => {
  ...
  it("still intercepts dotless workspace file hrefs when a file opener is provided", () => {
    ...
    render(
      <Markdown
        value="See [license](/workspace/AgentMonitor/LICENSE)"
        className="markdown"
        workspacePath="/Users/sotiriskaniras/Documents/Development/Forks/AgentMonitor"
        onOpenFileLink={onOpenFileLink}
      />,
    );

    const link = screen.getByText("license").closest("a");
    expect(link?.getAttribute("href")).toBe("/workspace/AgentMonitor/LICENSE");
    ...
    fireEvent(link as Element, clickEvent);
    expect(clickEvent.defaultPrevented).toBe(true);
    expectOpenedFileTarget(onOpenFileLink, "/workspace/AgentMonitor/LICENSE");
  });
  ...
  it("still intercepts nested workspace file hrefs when a file opener is provided", () => {
    ...
    render(
      <Markdown
        value="See [src](/workspaces/team/AgentMonitor/src)"
        className="markdown"
        workspacePath="/Users/sotiriskaniras/Documents/Development/Forks/AgentMonitor"
        onOpenFileLink={onOpenFileLink}
      />,
    ...
    expect(link?.getAttribute("href")).toBe("/workspaces/team/AgentMonitor/src");
    ...
    fireEvent(link as Element, clickEvent);
    expect(clickEvent.defaultPrevented).toBe(true);
    expectOpenedFileTarget(onOpenFileLink, "/workspaces/team/AgentMonitor/src");
  });
  ...
  it("intercepts Windows absolute file hrefs with #L anchors and preserves the tooltip", () => {
    ...
    expect(link?.getAttribute("href")).toBe(
      "I:%5Cgpt-projects%5CAgentMonitor%5Csrc%5Cfeatures%5Csettings%5Ccomponents%5Csections%5CSettingsDisplaySection.tsx#L422",
    );
    expect(link?.getAttribute("title")).toBe(
      "I:\\gpt-projects\\AgentMonitor\\src\\features\\settings\\components\\sections\\SettingsDisplaySection.tsx:422",
    );
    ...
    expect(onOpenFileLinkMenu).toHaveBeenCalledWith(
      expect.anything(),
      {
        path: "I:\\gpt-projects\\AgentMonitor\\src\\features\\settings\\components\\sections\\SettingsDisplaySection.tsx",
        line: 422,
        column: null,
      },
    );
